...
\begin{frame}{The Intuition Behind Vector Quantization}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \vspace{0.4em}
      \begin{itemize}
        \item Scalar 2-bit: \textbf{4} distinct values
        \item VQ ($v=2$, $k=4$): \textbf{4} distinct 2D vectors\\
              $\rightarrow$ covers a much wider 2D space
        \vspace{0.4em}
        \item $v=8$, $k=256$: 256 distinct 8D vectors\\
              $\approx$ effective \textbf{1 bit/weight}
      \end{itemize}
      \vspace{0.3em}
      \small Exploits correlations across\\weights for richer representation
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Computing the Hessian in Practice}
  ...
  \vspace{0.3em}
  \textbf{Key point}: $H = 2\mathbf{X}\mathbf{X}^T$ tells us \emph{which directions in weight space matter}
  \begin{itemize}
    \item Directions where $\mathbf{X}$ has high variance $\;\rightarrow\;$ large $H_{ii}$ $\;\rightarrow\;$ sensitive weights
    \item Directions where $\mathbf{X}$ is near zero $\;\rightarrow\;$ small $H_{ii}$ $\;\rightarrow\;$ safe to quantize coarsely
  \end{itemize}
\end{frame}
...
\begin{frame}{Component 1: The Local Quantization Rule}
  \begin{itemize}
...
\begin{frame}{Component 1: Where the Hessian is Still Used}
  \begin{itemize}
...
\begin{frame}{Component 1: Why This Helps Over GPTVQ}
  \begin{itemize}
    \item \textbf{GPTVQ}: quantizes multiple columns together
    \begin{itemize}
      \item Internal errors within the current block accumulate before correction
      \item Longer vectors become difficult to use
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{VPTQ}: quantizes one column, then immediately updates the remaining weights
    \begin{itemize}
      \item Less accumulation of local quantization error
      \item Better compatibility with longer vector lengths
      \item Simple nearest-centroid assignment instead of expensive training-based optimization
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{Practical result}: VPTQ reports using only \textbf{10.4--18.6\%} of the quantization time of prior SOTA methods while achieving better 2-bit accuracy
  \end{itemize}
\end{frame}
...
\begin{frame}{Component 2: Hessian-Weighted Centroid Initialization}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \small
      Standard K-means centroid A:\\
      $\frac{0.04+0.06}{2} = 0.05$\\[0.5em]
      \textbf{Weighted} centroid A:\\
      $\frac{10.0\times0.04\;+\;8.0\times0.06}{10.0+8.0} = \frac{0.88}{18} \approx 0.049$\\[0.5em]
