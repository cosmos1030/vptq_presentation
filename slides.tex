  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        ...
        \vspace{0.5em}
        \item 2-bit $\rightarrow$ only \textbf{4 distinct values}: $\{-1, 0, 1, 2\}$
        \vspace{0.5em}
...
\begin{frame}{Scalar Quantization vs.\ Vector Quantization}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{Scalar Quantization}\\[0.4em]
      \begin{tikzpicture}[scale=1.05]
        % grid lines
        \foreach \x in {-1.5,-0.5,0.5,1.5} {
          \draw[gray!40, thin] (\x,-1.9) -- (\x,1.9);
        }
        \foreach \y in {-1.5,-0.5,0.5,1.5} {
          \draw[gray!40, thin] (-1.9,\y) -- (1.9,\y);
        }
        \draw[gray!70] (-1.9,-1.9) rectangle (1.9,1.9);

        % actual weight vectors (true distribution - diagonal cluster)
        \foreach \x/\y in {
          -1.3/-1.1, -1.1/-0.9, -0.9/-1.2, -0.8/-0.7,
          -0.3/-0.2, -0.2/-0.4, -0.1/-0.1,  0.1/-0.3,
           0.7/ 0.6,  0.6/ 0.8,  0.9/ 0.7,  0.8/ 0.5,
           1.2/ 1.3,  1.4/ 1.1,  1.1/ 1.4
        } {
          \fill[black!50] (\x,\y) circle (1.8pt);
        }

        % quantized positions = nearest grid point, shown as red crosses
        \foreach \x/\y in {
          -1.5/-1.5, -0.5/-0.5, 0.5/0.5, 1.5/1.5
        } {
          \draw[mycolor, thick] (\x-0.12,\y-0.12) -- (\x+0.12,\y+0.12);
          \draw[mycolor, thick] (\x-0.12,\y+0.12) -- (\x+0.12,\y-0.12);
        }

        % arrows from a sample point to its quantized grid point
        \draw[->, mycolor!70, thin] (0.7,0.6) -- (0.5,0.5);
        \draw[->, mycolor!70, thin] (-1.3,-1.1) -- (-1.5,-1.5);
      \end{tikzpicture}\\[0.3em]
      \small Centroids \textcolor{mycolor}{$\times$} locked to grid intersections\\
      \textcolor{mycolor}{$\rightarrow$ large error where data is dense off-grid}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{Vector Quantization}\\[0.4em]
      \begin{tikzpicture}[scale=1.05]
        \draw[gray!70] (-1.9,-1.9) rectangle (1.9,1.9);
        ...
        % learned centroids placed freely at cluster centers
        \foreach \x/\y in {
          -1.025/-1.0,  -0.125/-0.25,  0.75/0.65,  1.225/1.275
        } {
          % Voronoi-ish region hint
          \fill[mycolor!20, opacity=0.5] (\x,\y) circle (0.38);
          \fill[mycolor] (\x,\y) circle (3.5pt);
        }

        % arrows from sample points to their nearest centroid
        \draw[->, mycolor!70, thin] (0.7,0.6) -- (0.75,0.65);
        \draw[->, mycolor!70, thin] (-1.3,-1.1) -- (-1.025,-1.0);
      \end{tikzpicture}\\[0.3em]
      \small Centroids \textcolor{mycolor}{$\bullet$} placed freely by learning\\
      \textcolor{mycolor!70!black}{$\rightarrow$ centroids sit at actual data clusters}
    \end{column}
  \end{columns}
  \vspace{0.2em}
  \centering\small
  Each axis = one weight dimension.\quad
  Both use the same number of bits; VQ uses them where the data actually lives.
\end{frame}
...
\begin{columns}[T]
  %% ---- GPTVQ ----
  \begin{column}{0.48\textwidth}
    \centering\textbf{GPTVQ}: quantize $v$ columns jointly
    \vspace{0.4em}

    \begin{tikzpicture}[scale=0.95]
      % column labels A, B only
      \foreach \c/\lbl in {0/A, 1/B} {
        \node[font=\small\bfseries] at (0.6+\c*1.4, 3.5) {\lbl};
      }
      % base grid: 2 cols x 3 rows
      \foreach \r in {0,1,2} {
        \foreach \c in {0,1} {
          \draw[gray!40] (\c*1.4,\r) rectangle (\c*1.4+1.4,\r+1);
        }
      }
      % A,B jointly quantized (orange)
      \fill[orange!35] (0,0) rectangle (2.8,3);
      \draw[orange!80!black, thick] (0,0) rectangle (2.8,3);
      \draw[orange!60, thin] (1.4,0) -- (1.4,3);
      % error labels — centred vertically, no overlap
      \node[font=\normalsize, mycolor] at (0.7, 1.5) {$\varepsilon_A$};
      \node[font=\normalsize, mycolor] at (2.1, 1.5) {$\varepsilon_B$};
      % "quantized together" label above grid
      \node[orange!80!black, font=\scriptsize\bfseries] at (1.4, 3.85) {quantized together};
      % error arrow pointing right, below grid
      \draw[->, very thick, mycolor] (2.85, 1.5) -- (3.8, 1.5)
        node[midway, above, font=\scriptsize, mycolor] {$\varepsilon_A{+}\varepsilon_B$};
      \node[mycolor, font=\scriptsize] at (4.3, 1.5) {\dots};
    \end{tikzpicture}

    \vspace{0.2em}
    \small\textcolor{mycolor}{$\varepsilon_A$ contaminates $B$ before it is quantized\\
    $\Rightarrow$ combined error propagates to all later columns}
  \end{column}

  %% ---- VPTQ ----
  \begin{column}{0.48\textwidth}
    \centering\textbf{VPTQ}: one column at a time
    \vspace{0.4em}

    \begin{tikzpicture}[scale=0.95]
      % column labels A, B only
      \foreach \c/\lbl in {0/A, 1/B} {
        \node[font=\small\bfseries] at (0.7+\c*1.4, 3.5) {\lbl};
      }
      % base grid: 2 cols x 3 rows
      \foreach \r in {0,1,2} {
        \foreach \c in {0,1} {
      ...
      % A: quantized (red)
      \fill[mycolor!22] (0,0) rectangle (1.4,3);
      \draw[mycolor, thick] (0,0) rectangle (1.4,3);
      \node[mycolor, font=\small] at (0.7, 1.5) {$\varepsilon_A$};
      % correction arrow from A to B (thin, dashed — internal correction)
      \draw[->, mycolor!50, dashed, thin] (1.42, 2.5) -- (2.58, 2.5)
        node[midway, above, font=\scriptsize, mycolor!70] {$-\varepsilon_A$};
      % B: adjusted then quantized (green)
      \fill[green!18] (1.4,0) rectangle (2.8,3);
      \draw[green!60!black, thick] (1.4,0) rectangle (2.8,3);
      \node[green!50!black, font=\small] at (2.1, 1.5) {$\varepsilon_B'$};
      % outgoing arrow from B — only epsilon_B' propagates
      \draw[->, thick, green!60!black] (2.85, 1.5) -- (3.55, 1.5)
        node[midway, above, font=\scriptsize, green!50!black] {$\varepsilon_B'$};
      \node[gray!60, font=\normalsize] at (3.85, 1.5) {\dots};
    \end{tikzpicture}

    \vspace{0.2em}
    \small\textcolor{mycolor!70!black}{Only $\varepsilon_B'$ propagates — much smaller than $\varepsilon_A{+}\varepsilon_B$}
  \end{column}
\end{columns}
\vspace{0.15em}
\centering\small\textbf{Key}: Approximate diagonal dominance allows simpler column-wise optimization. $\Rightarrow$ columns can be treated independently
\end{frame}
